...
#define BORDER *((volatile uint32_t *)0xFEC80008)
#define DONT_OPTIMIZE_OUT 	  __asm__ __volatile__("");
...
void superio_init(void) {
	...
	BORDER = 0xff00dd; // Fushia si on est sorti de UnReset_LPC
	
	configure_zones();

	BORDER = 0x003300; /// Vert foncé si on est sorti de configure_zones
...
void UnReset_LPC( ) {
  unsigned int i;
  GABE_CTRL_LPC |= 0x100; // This will set the LPC_RSTn which is its normal operation state.
  for (i = 0; i< 1000; i++)
	  DONT_OPTIMIZE_OUT;

  GABE_CTRL_LPC &= 0xFEFF; // This will set the LPC_RSTn which is its normal operation state.
  for (i = 0; i< 400000; i++)
	  DONT_OPTIMIZE_OUT;
...
static void set_base_address(uint16_t adr) {
    ...
    *CONFIG_0x2F_REG = (adr>>8);
    *CONFIG_0x2E_REG = 0x61; /* Low*/
    *CONFIG_0x2F_REG = (uint8_t)(adr&0xff);
}
